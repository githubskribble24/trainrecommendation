...
public:
   ...
   *
   *  \returns  false if there was an error while adding the line to the network
   * 
   ...
   *  line cannot already be in th network
   */
  bool AddLine(
    const Line& line
  );
   ...
   *           the travel times
   *  
   *  \throws  std::rumtime_errror This method throws if the JSON items were
   *                               parsed correctly but there was an issue
   *                               adding new stations or lines to the network.
   *  \throws  nlohman::json::exception If there was a problem parsing the
   *                                    JSON object
   */
  bool FromJson(
    nlohmann::json&& src
...
private:
  ...
  // We map routes by their route ID
  struct RouteInternal
  {
    Id id {};
